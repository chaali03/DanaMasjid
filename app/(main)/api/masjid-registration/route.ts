...
const pool: Pool = globalForPg.pgPool ?? new Pool({ 
  connectionString: process.env.DATABASE_URL,
  // Add idle timeout and max connections for stability
  max: 20,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, token, ...formData } = body

    if (!userId) return NextResponse.json({ error: 'userId diperlukan' }, { status: 400 })
    if (!token) return NextResponse.json({ error: 'token pendaftaran diperlukan' }, { status: 400 })
    
    // Verify token session one last time before final submission
    const { prisma } = await import('@/lib/prisma')
    
    if (!prisma.registrationToken) {
      return NextResponse.json({ error: 'Sistem pendaftaran belum siap' }, { status: 503 })
    }
    ...
    if (!tokenRecord || tokenRecord.isUsed) {
      return NextResponse.json({ error: 'Token pendaftaran tidak valid atau sudah digunakan' }, { status: 400 })
    }

    const now = new Date()
    if (new Date(tokenRecord.expiresAt) < now) {
      return NextResponse.json({ error: 'Token sudah hangus (melebihi 7 hari)' }, { status: 403 })
    }
    if (tokenRecord.sessionExpiry && new Date(tokenRecord.sessionExpiry) < now) {
      return NextResponse.json({ error: 'Sesi pengisian formulir berakhir (melebihi 1 jam)' }, { status: 401 })
    }
    ...
    // Mark token as used upon successful registration

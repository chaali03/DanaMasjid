...
export async function POST(request: NextRequest) {
  try {
    ...
    // Store token in Prisma
    const { prisma } = await import('@/lib/prisma')
    
    if (!prisma.registrationToken) {
      return NextResponse.json(
        { success: false, message: 'Sistem pendaftaran sedang dalam pemeliharaan' },
        { status: 503 }
      )
    }
    ...
    await prisma.registrationToken.create({
      data: {
        ...
        expiresAt,
        sessionExpiry: new Date(Date.now() + 1 * 60 * 60 * 1000), // 1 hour session
      }
    })

    // Store token in Firestore using Admin SDK (for backward compatibility if needed)
    try {
      ...
      console.warn('Firestore fallback failed, continuing with Prisma')
    }

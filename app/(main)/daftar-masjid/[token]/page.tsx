...
export default function DaftarMasjidPage() {
  ...
  useEffect(() => {
    ...
    const verifyToken = async () => {
      try {
        ...
        if (!result.success) {
          if (result.action === 'VERIFY_DEVICE') {
            toast.error(result.message)
            router.replace(`/verify-device?redirect=/daftar-masjid/${token}`)
          } else if (result.action === 'LOGIN') {
            toast.error(result.message)
            router.replace(`/login?redirect=/daftar-masjid/${token}`)
          } else {
            toast.error(result.message || 'Token tidak valid')
        ...
        toast.error('Gagal memverifikasi link pendaftaran')
      } finally {
  ...
  useEffect(() => {
    if (!mounted || verifyingToken || !tokenVerified) return
    ...
    const grace = setTimeout(() => {
      ...
      if (uid) {
        ...
        // Double check if user still null after grace period
        if (!user && !authLoading) {
          router.replace(
            `/login?redirect=/daftar-masjid/${token}&message=Harus+login+terlebih+dahulu+untuk+mendaftar+masjid&type=daftar-masjid`
          )
        }
      }
    }, 2000) // Increased to 2 seconds for better stability on slow networks

    return () => clearTimeout(grace)
  }, [user, authLoading, mounted, router, verifyingToken, tokenVerified, token])
  ...
  useEffect(() => {
    ...
    const timeoutCheck = setInterval(() => {
      ...
      if (inactiveTime > 60 * 60 * 1000) {
        ...
        localStorage.setItem('redirect_after_login', `/daftar-masjid/${token}`)
        router.push('/login')
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!userId) {
        ...
        router.push(`/login?redirect=/daftar-masjid/${token}`)
        return
      }
  ...
  // Final loading state
  if (!mounted || authLoading || verifyingToken || !authChecked || !tokenVerified) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          <LottieLoading className="bg-transparent h-auto" hideText />
          <p className="mt-4 text-slate-500 font-medium">
            {!tokenVerified ? 'Memverifikasi link pendaftaran...' : 'Menyiapkan formulir pendaftaran...'}
          </p>
        </div>
      </div>
    )

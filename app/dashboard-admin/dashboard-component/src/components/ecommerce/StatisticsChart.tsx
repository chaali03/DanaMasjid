...
function StatisticsChartComponent() {
  ...
  const options: ApexOptions = useMemo(() => ({
    ...
    colors: [colors.lineChart.primary, colors.lineChart.secondary], // Use custom colors
    chart: {
    ...
    <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      ...
      <div className="max-w-full overflow-x-auto custom-scrollbar">
        <div className="min-w-[1000px] xl:min-w-full">
          <Chart 
            key={`statistics-${colors.lineChart.primary}-${colors.lineChart.secondary}`}
            options={options} 
            series={series} 
            type="area" 
            height={310} 
...
export default memo(StatisticsChartComponent);
